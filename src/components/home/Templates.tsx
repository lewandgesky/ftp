...
const TEMPLATES: Template[] = [
  {
    ...
    abstract:
      "Ce rapport examine en profondeur les processus de gestion de la chaîne logistique au sein de l'entreprise XYZ, avec une attention particulière portée sur l'optimisation des flux de marchandises et la réduction des coûts opérationnels.",
    sections: [
  ...
  },
  {
    ...
    abstract:
      "Ce projet de stage porte sur la conception et le développement d'une application web de gestion des stocks pour la société ABC, utilisant les technologies modernes React et Node.js pour créer une solution performante et intuitive.",
    sections: [
      "Introduction",
      "Étude de l'existant",
  ...
  },
  {
    ...
    abstract:
      "Cette étude analyse les stratégies de communication digitale mises en œuvre lors du stage au sein de l'agence DEF, incluant la gestion des réseaux sociaux, la création de contenu et l'analyse des KPIs de performance.",
    sections: [
  ...
  },
  {
    ...
    abstract:
      "Ce rapport de stage détaille les missions d'audit financier et de contrôle de gestion réalisées au sein du cabinet GHI, couvrant l'analyse des états financiers, l'évaluation des risques et les procédures de contrôle interne.",
    sections: [
      "Introduction",
...
/* Carte réutilisable (grille ET scroll horizontal) */
function TemplateCard({
  ...
  return (
    <Card
      ...
      className={`overflow-hidden group card-hover bg-white border-[#d4cdc5] cursor-pointer ${className ?? ""}`}
    >
      <div
        className={`h-44 bg-gradient-to-br ${template.color} border-b ${template.border} relative flex items-center justify-center p-4`}
      >
        ...
        <div className="relative z-10 w-full h-full bg-white rounded shadow-lg flex flex-col p-3 overflow-hidden">
          ...
          <div className="text-[6px] text-gray-800 font-bold text-center mb-1.5 leading-tight">
            {template.title}
          </div>
...
export function Templates() {
  ...
  // Détection desktop pour activer le scroll horizontal pinné
  ...
  // Mesures réelles pour le scroll horizontal
  ...
  useEffect(() => {
    ...
    // Recalcule une fois les images/polices posés
  ...
  const useHorizontal = isDesktop && !reduceMotion;
  ...
  return (
    <>
      <section
        id="templates"
        ref={targetRef}
        className={`relative bg-[#ebe5de]/80 backdrop-blur-md border-y border-white/20 ${
          useHorizontal ? "" : "py-20"
        }`}
        style={useHorizontal ? { height: `calc(100vh + ${maxScroll}px)` } : undefined}
      >
        {useHorizontal ? (
          /* ===== Mode scroll horizontal pinné (desktop) ===== */
          <div className="sticky top-0 h-screen flex flex-col justify-center overflow-hidden">
            <div className="container mx-auto px-4 md:px-6 mb-10">
              <motion.h2
                ...
                className="text-3xl md:text-4xl font-bold mb-4 text-[#1e2d3d]"
              >
                {t("templates.title")}
              </motion.h2>
              <motion.p
            ...
            <motion.div
              ref={trackRef}
              style={{ x }}
              className="flex gap-6 px-6 lg:px-16 will-change-transform"
            >
              {TEMPLATES.map((template) => (
                <div
                  key={template.id}
                  className="shrink-0 w-[78vw] sm:w-[55vw] lg:w-[34vw] xl:w-[28vw]"
                >
                  <TemplateCard
                    template={template}
              ...
              {/* Espace final pour la dernière carte */}
              <div className="shrink-0 w-[8vw]" aria-hidden />
            </motion.div>

            {/* Indicateur "scroll" */}
            <div className="container mx-auto px-4 md:px-6 mt-8 flex items-center gap-3 text-[#9ca3af]">
              <span className="text-xs uppercase tracking-widest">
                Scrollez pour explorer
              </span>
              <div className="flex-1 max-w-[160px] h-[2px] bg-[#d4cdc5] rounded-full overflow-hidden">
                <motion.div
                  style={{ scaleX: scrollYProgress }}
                  className="h-full origin-left bg-gradient-to-r from-[#c8944e] to-[#d4a96a]"
                />
              </div>
            </div>
          </div>
        ) : (
          /* ===== Mode grille (mobile / reduced motion) ===== */
        ...
        {selectedTemplate && (
          ...
          >
            <motion.div
              ...
              className="bg-white border border-[#d4cdc5] rounded-2xl w-full max-w-3xl max-h-[85vh] overflow-auto shadow-2xl"
            ...
            >
              ...
              <div className="flex items-center justify-between p-6 border-b border-[#d4cdc5]">
                <div>
                  ...
                  <h3 className="text-xl font-bold mt-3 text-[#1e2d3d]">
                    {selectedTemplate.title}
                  </h3>
                  <p className="text-sm text-[#6b7b8d] mt-1">
                    {selectedTemplate.pages} pages
                  </p>
                </div>
                <button
                  onClick={() => setSelectedTemplate(null)}
              ...
              <div className="p-6">
                <div className="bg-white rounded-lg shadow-lg p-8 md:p-12 text-gray-800 max-w-2xl mx-auto">
                  ...
                  {/* Abstract */}
                  <div className="mb-8">
                    <h5 className="text-sm font-bold uppercase tracking-wide text-gray-500 mb-3">
                      Résumé
                    </h5>
                    <p className="text-sm text-gray-600 leading-relaxed">
                      {selectedTemplate.abstract}
                    </p>
                  </div>

                  {/* Table of contents */}
                  <div>
                    <h5 className="text-sm font-bold uppercase tracking-wide text-gray-500 mb-4">
                      Table des matières
                    </h5>
                    <div className="space-y-2">
                      {selectedTemplate.sections.map((section, i) => (
                        <div
                          key={i}
                          className="flex items-center justify-between text-sm py-1.5 border-b border-dashed border-gray-200"
                        >
                ...
                <div className="text-center mt-8">
                  <p className="text-[#6b7b8d] text-sm mb-4">
                    Vous voulez un rapport similaire ?
                  </p>
                  <Link href="/commander">
